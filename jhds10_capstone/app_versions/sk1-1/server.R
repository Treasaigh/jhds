...
make_prediction_vector <- function(user_input, num_pred=1) {
  ...
  rec_queue[1:num_pred]
}


shinyServer(
    function(input,output) {
        output$rec1 <- renderPrint({make_prediction_vector(input$input_text,input$num_pred)})
    }
)
